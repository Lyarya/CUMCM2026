...
同口径审计结果如表\ref{tab:p1-lp-relaxation}所示。
\input{contents/generated/q1_lp.tex}

\subsection{效率修正的局部套利判据}
考虑仅由价差驱动的两个时段 $i$ 和 $j$。若时段 $i$ 从外网额外购买 $1\,\mathrm{kWh}$ 用于充电，则进入储能 $\eta_c\,\mathrm{kWh}$，未来可向负荷释放 $\eta_c\eta_d\,\mathrm{kWh}$。暂不考虑功率和电量边界时，储能通过跨时段转移获得正收益需要满足
\begin{equation}
p_j\eta_c\eta_d>p_i,
\qquad
...
\end{equation}
当 $\eta_c=\eta_d=0.9$ 时，临界价格比约为 1.235。式\eqref{eq:p1-arbitrage-threshold}给出了考虑充放电损耗后的基本价差条件；在完整调度中，SOC、功率上限、光伏出力、负荷需求和终端电量约束还会共同影响储能的实际充放电时机。图\ref{fig:p1-dispatch}显示，储能主要在低价或光伏富余时段充电，并在较高电价时段释放电量，与上述价差机制相吻合。
...
为考察储能关键参数变化对调度结果的影响，我们采用单因素分析。令充、放电效率满足 $\eta_c=\eta_d\in\{0.80,0.85,0.90,0.95\}$，容量和功率上限分别取基准值的 $0.50,\allowbreak0.75,\allowbreak1.00,\allowbreak1.25$ 倍，其余条件保持不变。调整容量时，电量下界、上界及首末时刻储能电量分别设为容量的 10\%、90\% 和 50\%。共得到 12 组参数情形，均成功求得最优解并通过物理约束核验，结果如图\ref{fig:p1-sensitivity} 所示。
\begin{figure}[H]
\centering
...
\end{figure}
结果表明，提高储能效率能够显著降低购电费用：单程效率由 $0.80$ 提高至 $0.95$ 时，全天购电费用下降 $11.66\%$。扩大储能容量同样能够降低费用，但节费效果逐渐减弱；容量每增加 $3000\,\mathrm{kWh}$，对应节费依次为 $2850.82$、$2042.64$ 和 $1570.20$ 元。相比之下，提高充放电功率上限的作用较快趋于饱和，每增加 $1250\,\mathrm{kW}$ 所带来的节费依次为 $1043.97$、$219.92$ 和 $53.93$ 元，在 $5000\,\mathrm{kW}$ 之后继续提高功率上限的改善已十分有限。综合来看，本算例中储能效率对运行费用的影响最明显，增加容量仍具有一定收益，而进一步提高功率上限的作用较小。

问题一建立的储能调度模型构成后续各问的基础，问题二至问题四将在此基础上依次引入预测不确定性、滚动调整和动态电价。
