...
$s=1,\ldots,50$ 个成对负荷--光伏场景，另定义紧急购电功率 $e_{t,s}$ 与弃光功率
$w_{t,s}$。本文将 50 个历史整日残差自助场景视为等概率样本，负荷和光伏始终使用同一场景
索引。基础期望成本模型为
\begin{equation}
  \min \quad
  ...
  +\rho\left|E_{145}-E_1\right|,
  \label{eq:q2-expected-cost}
\end{equation}
其中 $\Delta t=1/6\ \mathrm{h}$，紧急购电按交易时刻普通电价的 5 倍结算。绝对值通过两个
非负辅助变量线性化。其权重取
$\rho=\eta_d\max_t\lambda_t=1.25568\ \text{元}/\mathrm{kWh}$，用一单位内部储能在下一峰价时段的
机会价值上界抑制有限日视界末端放空。

每个场景均满足
\begin{equation}
  g_t+e_{t,s}+P^{\mathrm{pv}}_{t,s}+d_t
  =L_{t,s}+c_t+w_{t,s},
  \qquad 0\le w_{t,s}\le P^{\mathrm{pv}}_{t,s},\quad e_{t,s}\ge0.
  \label{eq:q2-balance}
\end{equation}
储能约束沿用问题一的物理参数：
\begin{align}
...
$E_{d+1,1}=E_{d,145}$。软终端项以当天实际带入的 $E_{d,1}$ 为参考，并不会把状态重置为常数。

按日期顺序求解 2025 年 2 月 1 日至 12 月 31 日的 334 个日模型，PuLP--HiGHS 在全部日期
均返回最优状态。累计计划购电费用为 $11402778.053$ 元，模型内的期望紧急购电费用为
$7936334.113$ 元。计划确定后，再以附件 2 的实际负荷和实际光伏逐时结算，实际紧急购电量为
$2341013.444\ \mathrm{kWh}$，相应紧急购电费为 $8438058.143$ 元，实际总购电费为
$19840836.195$ 元。全期最大场景功率平衡残差为
$3.10\times10^{-11}\ \mathrm{kW}$，最大储能递推残差为
$5.01\times10^{-12}\ \mathrm{kWh}$。正式数据下软终端项的最优日末电量均为
$6000\ \mathrm{kWh}$；该数值由目标函数与数据共同产生，而非每日首尾相等约束。
